...
BATCH_SIZE = 4
LEARNING_RATE = 1e-4
KL_WEIGHT = 1e-6        # > 0 -> VAE. 0 -> determinisztikus AE.
...
def spawn_at_free_point(world, bp, spawn_points):
    ...
    for point in random.sample(spawn_points, len(spawn_points)):
        ...
        # Town04 nem sik - a feluljarokon a spawn pontok 10+ meteren vannak.
...
def spawn_traffic(client, world, count=None):
    """
    Lerak `count` autot autopilotban (None = ahany spawn pont van).
    Visszaadja a listajukat.
    """
    tm = client.get_trafficmanager()
    tm.set_global_distance_to_leading_vehicle(2.5)

    # HYBRID PHYSICS MODE - enelkul ennyi auto nem futna
    #
    # Alapertelmezesben a CARLA MINDEN autora teljes kerekfizikat szamol
    # (felfuggesztes, gumi-tapadas, motor). Par tucat auto folott ez megfogja a
    # szimulaciot: a szerver FPS leesik, az autopilot kesve reagal - innen a
    # rangatas es a koccanasok.
    #
    # Hybrid modban csak a HERO korul HYBRID_PHYSICS_RADIUS meteren belul van
    # teljes fizika. A tavolabbi autokat a TM olcso "teleportalos" modban
    # mozgatja: tolja oket a sav menten, kerekszamitas nelkul. Ranezesre
    # ugyanugy kozlekednek, csak toredek koltseggel - igy fer el a palya
    # osszes spawn pontjan auto.
    #
    # A hero-t a role_name="hero" attributum jeloli ki (lasd Ego.spawn());
    # enelkul a TM nem tudna, mi kore rajzolja a kort.
    ...
    # ~370 autonal 370*370 probalkozas lett volna.
    if count is None:
...
def points_to_range_image(xyz):
    """
    Nyers XYZ pontfelho -> (1, 64, 1024) float32 range image, [-1, 1].

    Ket geometry.py fuggveny osszefuzese:
      pcd2range    - gombi vetites; a cellakba a TAVOLSAGOT irja (-1 ahol ures)
      process_scan - log skalazas + normalizalas [-1, 1]-be

    A log skalazas azert kell, mert a kozeli tartomany a fontos: 5 es 10 meter
    kozott sokkal nagyobb kulonbseg van vezetes szempontjabol, mint 45 es 50
    kozott. A log2 pont ezt a felbontast adja a kozelbe.
    """
...
def range_to_surface(img, width, height):
    """
    (1, 64, 1024) range image [-1, 1] -> pygame surface.

    Szinskala szurkearnyalat helyett: a kozeli pontok pirosak, a tavoliak
    kekek, az ures cellak feketek. Igy szabad szemmel is megitelheto, hogy a
    rekonstrukcio eltalalja-e a TAVOLSAGOT, nem csak azt, hogy van-e ott
    egyaltalan valami.
    """
...
def bev_to_surface(bev, width, height):
    """A HUD felulnezeti kepe. Ezen NEM tanul a halo, csak neked mutatja meg,
    mi van az auto korul - a range image ranezesre nehezen olvashato."""
...
class Ego(object):
    ...
    def spawn(self):
        ...
        if self.player is None:
            # Tele palyan (minden spawn ponton all egy auto) ez elofordul, es
            # tipikusan a BACKSPACE-szel valo ujraspawnnal. A palyan levo
            # autokat a szimulacio mozgatja, tehat par tizedmasodperc mulva mar
            # szabad lehet egy pont - ezert varunk es ujraprobalunk, nem
            # dobunk azonnal hibat.
            for _ in range(5):
                time.sleep(0.4)
                self.player = spawn_at_free_point(
                    self.world, bp, self.world.get_map().get_spawn_points())
        ...
        # A wrappers.Lidar a world-ot varja, nem a carla.World-ot: a
        # CarlaActorBase az actor_list-be regisztral. Itt nincs env, ezert egy
        # minimalis shim megy be helyette (lasd WorldShim).
    ...
    def _on_camera(self, image):
        ...
        # kepbuffereket, tehat masolas nelkul a tartalom barmikor
        # felulirodhat alattunk - epp rajzolas kozben is.
        bgra = np.reshape(np.frombuffer(image.raw_data, dtype=np.uint8),
                          (image.height, image.width, 4))
        self.viewer_image = bgra[:, :, :3][:, :, ::-1].copy()

    def get_carla_actor(self):
        """A Lidar wrapper ezt hivja az attach_to-n."""
        return self.player
...
class WorldShim(object):
    """
    Minimalis "world", amit a wrappers.CarlaActorBase elvar.

    A wrappers.py osztalyai egy sajat World objektumot varnak, aminek van
    actor_list-je es map-je. Itt nincs env, ezert a carla.World-ot kiegeszitjuk
    ezzel a ketto mezovel - igy a Lidar wrapper valtoztatas nelkul hasznalhato.
    """
...
class Trainer(object):
    ...
    Miert kell a kulon szal: egy gradiens lepes (batch=4) ~400 ms egy RTX 4070
    Laptopon, a rekonstrukcio ~25 ms. A rajzolo ciklus 30 FPS-t celoz, az
    33 ms/kepkocka - ha a tanitas ott futna, a kep 2-3 FPS-re esne, es a
    billentyuk is akadoznanak, mert a pygame esemenyek sem jutnanak szohoz.

    Igy viszont a fo szal csak rajzol, a halo pedig a hatterben porog. A GIL
    nem gond: a PyTorch a CUDA hivasok es a nagy tenzormuveletek idejere
    elengedi, tehat a ket szal tenylegesen parhuzamosan halad.
    """

    def __init__(self, device):
        self.device = device
        self.model = LidarAE(ddconfig=DDCONFIG, learning_rate=LEARNING_RATE,
                             kl_weight=KL_WEIGHT).to(device)
        ...
        # deque maxlen-nel: a legregibb kep automatikusan kiesik, ha megtelt.
        # Ez tartja korlatosan a memoriat, es egyben "csuszoablakot" ad -
        # mindig a legutobbi ~512 kepbol tanulunk.
        #
        # A deque append muvelete onmagaban szalbiztos, de a random.sample nem:
        # kozben valtozhat a meret. Ezert a lock.
        self.buffer = deque(maxlen=BUFFER_SIZE)
        self.lock = threading.Lock()
        # Kulon lock a mentesnek: a buffer-lockot nem hasznalhatjuk erre, mert
        # azt a lidar callback is varna, es a mentes idejere kepeket vesztenenk.
        ...
        # A legutobbi be/ki kep a HUD-nak. Ezeket a tanito szal irja, a fo szal
        # olvassa. Egy referencia-ertekadas atomi, es nem baj, ha a fo szal egy
        # kepkockaval regebbit lat - ezert nem kell ide lock.
    ...
    def _train_step(self, batch):
        ...
        if posterior is not None:
            loss = loss + KL_WEIGHT * posterior.kl().mean()
