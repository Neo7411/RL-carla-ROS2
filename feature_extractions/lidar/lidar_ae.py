"""
LiDAR autoencoder - MINDEN EGY FAJLBAN, VAE nelkul.

Ez a fajl az AE/ mappa osszes halo-kodjat tartalmazza egyetlen helyen:
    geometria (pontfelho <-> range image)
    encoder   (graf-alapu, EdgeConv)
    decoder   (ResNet + felskalazas)
    a teljes modell

MI MARADT KI az eredeti (TopoLiDM) kodbol es miert:

  - VAE / DiagonalGaussianDistribution / kl_weight / double_z / posterior
        Determinisztikus autoencodert tanitunk. A VAE-agak minden `if`-je
        kikerult, nem csak a hasznalatuk - igy nincs holt kod.

  - pytorch_lightning (LightningModule, training_step, self.log, ...)
        A train.py sajat tanito ciklust futtat kulon szalon, a Lightningbol
        semmit nem hasznaltunk. Marad a sima nn.Module.

  - attention (make_attn, attn_levels)
        Az eredetiben SOSEM volt implementalva: a make_attn() minden agon
        nn.Identity()-t adott vissza. A config is ures listat adott neki.

  - temb (time embedding)
        Diffuzios modellek maradvanya. A decoderben vegig None volt, a
        temb_ch = 0, tehat a temb_proj reteg sem jott letre soha.

  - az encoder h2/h4 visszaterese
        Az eredeti topologiai losszhoz kellett, amit mar a korabbi verzio
        sem hasznalt. Most csak a latenst adjuk vissza.

  - range2pcd / get_depth_thresh / remission / labels
        A range image visszaalakitasa pontfelhove es a maszkolas; egyik sem
        kell a tanitashoz.

Fuggoseg: torch, numpy. Semmi mas.
"""
